...
const appConstants = require('../../utils/appConstant');
/* GET users listing. */
router.get('/', async (req, res, next) => {
  var data = await userService.getUsers();
  if( data ) return await res.status(200).send({ data });
});
...
router.post('/', async (req ,res) => {
  ...
  return await res.status(202).send({ data });
});

router.delete('/:id', async (req ,res) => {
  var data  =  await userService.deleteUser(req.params.id);
...
router.delete('/:userid/addresses/:id', async (req ,res) => {
  var data  =  await addressService.deleteAddress(req.params.id);
  return await res.status(200).send({ data });
})
